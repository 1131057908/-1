...
@file:time模块.PY
@ide:PyCharm
@time:2018-07-26 10:51:52
"""
import time

#1.获取时间戳
#1.获取时间戳，时间戳单位为秒，从1970年1月1日00:00到现在一共经历的时间是多少秒。
#时间戳一般用于验证登陆是否过期
timetamp=time.time()
print(timetamp)
...
# (tm_year=2018, tm_mon=7, tm_mday=26, tm_hour=14, tm_min=12, tm_sec=44, tm_wday=3, tm_yday=207)
#2018-7-26 11:01：xx
#strfrtime()：将时间元组转换成格式化的时间字符串
time_1=time.strftime("%Y-%m-%d %H:%M:%S",time.localtime())
print(time_1)

#5.strptime():将格式化的字符串转化成时间元组
